...
template<typename T>
void PermutohedralReverseCpu<T>::compute_Ldiff(const value_type* in, value_type* out) {
...
//PW
template <typename T>
void PermutohedralReverseCpu<T>::slice_Ldiff(
    const std::vector<value_type>& data,
    value_type* sliced) const {
  // data           num_output x M_                               row-major
  ...
  for (int i = 0; i < static_cast<int>(d_); i++) {
    ...
    for (int j = 0; j <= d_; j++) {
      //int o = lattice_->offset_[(out_offset_+i)*(d_+1)+j];
      //value_type w = lattice_->barycentric_Ldiff[(out_offset_+i)*(d_+1)+j];      
      int o = lattice_->offset_Ldiff_[( i)*(d_+1)+j];
      value_type w = lattice_->barycentric_Ldiff_[( i)*(d_+1)+j];

      for (int k = 0; k < static_cast<int>(num_output_); k++) {
        sliced[k*d_ + i] += 2 *w * data[k * M_ + o];
      }
    }
  }
}
...
void Permutohedral<T>::init(
  ...
  lattice->barycentric_Ldiff_.resize(static_cast<std::size_t>((d+1) * d));
  std::vector<boost::int16_t> ranks;
  ranks.resize((d+1) * N);
  lattice->offset_.resize((d+1) * N);
  lattice->offset_Ldiff_.resize((d+1) * d);
  ...
  std::vector<value_type> elevated_(d+1);
  std::vector<float> rem0_(d+1);
  std::vector<value_type> barycentric(d+2);
  std::vector<value_type> barycentric2(d+2);  
  std::vector<boost::int16_t> canonical((d+1)*(d+1));
  std::vector<boost::int16_t> key(d+1);
  std::vector<boost::int16_t> key_(d+1);
...
//PW
  ...
  for (int k = 0; k < d; k++) {
    ...
    int sum = 0;
    for (int i = 0; i <= d; i++) {
      int rd = round(down_factor * static_cast<float>(elevated_[i]));
      rem0_[i] = rd*up_factor;
      sum += rd;
    }
    ...
    boost::int16_t* rank = ranks.data() + (d+1)*k;
    for (int i = 0; i < d; i++) {
      double di = static_cast<float>(elevated_[i]) - rem0_[i];
      for (int j = i+1; j <= d; j++)
        if ( di < static_cast<float>(elevated_[j]) - rem0_[j])
          rank[i]++;
        else
          rank[j]++;
    }
    ...
    if (do_visualization){
      ...
      for (int i = 0; i <=d+1; i++)
        barycentric2[i] = 0;
      for (int i = 0; i <=d; i++) {
        value_type v = (elevated_[i] - rem0_[i])*down_factor;
        ...
        // assert(d_-rank[i]   >= 0);
        // assert(d_-rank[i]+1 <  d_+2);
        barycentric2[d-rank[i]  ] += v;
        barycentric2[d-rank[i]+1] -= v;
      }
      // Wrap around
      barycentric2[0] += 1.0 + barycentric2[d+1];
    }
