...
template <typename T>
void
PermutohedralReverseGpu<T>::init(
...
//***
  ...
  offset_Ldiff_.Reshape(1, 1, d_, d_ + 1);
  assert(lattice->offset_Ldiff_.size() == d_ * (d_ + 1));
  std::copy(lattice->offset_Ldiff_.begin(), lattice->offset_Ldiff_.end(),
            offset_Ldiff_.mutable_cpu_data());
  ...
  caffe_gpu_memcpy(filter_.count() * sizeof(value_type),
                   filter, filter_.mutable_gpu_data());
...
__global__ void copy_to_blur_data(const T* splat_data,
                                  const int M, const int num_output,
                                  T* blur_data) {
...
template<typename T>
void
PermutohedralReverseGpu<T>::compute(const value_type* in, value_type* out) {
...
template<typename T>
void
PermutohedralReverseGpu<T>::compute_Ldiff(const value_type* in, value_type* out) {
  Blob<value_type> blurred_(1, 1, M_, num_output_);
...
template<typename T>
void
PermutohedralReverseGpu<T>::max_compute(const value_type* in, value_type* out) {
  Blob<value_type> blurred_(1, 1, M_, num_output_);
...
template <typename T>
__global__ void slice_gpu_kernel(const int out_size, const T* data,
                                 const int M, const int d,
                                 const int out_offset,
                                 const int num_output,
                                 const int* offset, const T* barycentric,
                                 T* sliced) {
...
template <typename T>
void PermutohedralReverseGpu<T>::slice(
    const Blob<value_type>& data,
    value_type* sliced)
  const {
...
template <typename T>
void PermutohedralReverseGpu<T>::slice_Ldiff(
    const Blob<value_type>& data,
    value_type* sliced)
  const {
  // data           num_output x M_                               row-major
...
__global__ void im2col_gpu_kernel(
    const T* im, const int value_size,
    const int filter_size, const int M,
    const int start, const int output_size,
...
template <typename T>
void PermutohedralReverseGpu<T>::im2col(
  ...
  im2col_gpu_kernel<value_type>
    ...
    im, value_size, filter_size, M, start, output_size, blur_neighbors,
    col);
...
__global__ void col2im_gpu_kernel(
    const T* col, const int value_size,
    const int filter_size, const int M,
    const int start, const int output_size,
    const int* blur_neighbors, T* im) {
  CUDA_KERNEL_LOOP(i, output_size) {
    for (std::size_t k = 0; k < value_size; ++k) {
      im[k*(M + 1) + (i + start + 1)] +=
        col[(k * filter_size + 0) * output_size + i];
...
template <typename T>
void PermutohedralReverseGpu<T>::col2im(
  ...
  col2im_gpu_kernel<value_type>
    // NOLINT_NEXT_LINE(whitespace/operators)
    <<<CAFFE_GET_BLOCKS(output_size), CAFFE_CUDA_NUM_THREADS>>>(
        col, value_size, filter_size, M, start, output_size,
        blur_neighbors, im);
...
template <typename T>
void PermutohedralReverseGpu<T>::blur(
    const Blob<value_type>& splatted,
    const Blob<value_type>& filter, Blob<value_type>* blurred) const {
  ...
  if (size > 1)
    blur_neighbors = blur_neighbors_.gpu_data();
  ...
  for (std::size_t g = 0; g < group_; ++g) {
    for (std::size_t c = 0; c < chunks; ++c) {
      ...
      caffe_gpu_gemm_ex<value_type>(
          ...
          filter.gpu_data() + filter_offset * g, K,
          col_data, end - start,
...
template <typename T>
__global__ void maxpool_gpu_kernel(
    const T* splat_data, const int value_size,
    const int filter_size, const int M,
    const int* blur_neighbors, T* maxxed_data,
    int* idx_data) {
...
template <typename T>
void PermutohedralReverseGpu<T>::max(
    const Blob<value_type>&  splatted,
  ...
  maxpool_gpu_kernel<value_type>
    ...
    <<<CAFFE_GET_BLOCKS(value_size_), CAFFE_CUDA_NUM_THREADS>>>(
        splat_data, value_size_, filter_size, M_,
        blur_neighbors, max_data, max_idx_.mutable_gpu_data());

  CUDA_POST_KERNEL_CHECK;
}

template <typename T>
__global__ void splat_gpu_kernel(const int in_size, const T* in,
                                 const int M, const int d,
                                 const int in_offset,
...
template <typename T>
void PermutohedralReverseGpu<T>::splat(
    const value_type* in, Blob<value_type>* splatted) const {
  ...
  splat_gpu_kernel<value_type>
    // NOLINT_NEXT_LINE(whitespace/operators)
    <<<CAFFE_GET_BLOCKS(in_size_), CAFFE_CUDA_NUM_THREADS>>>(in_size_, in,
    M_, d_,
    in_offset_,
    value_size_,
    offset_.gpu_data(), barycentric_.gpu_data(),
...
__global__ void copy_to_blurred_out_data(const T* sliced_out,
                                  const int M, const int value_size,
                                  T* blurred_out) {
...
template <typename T>
void PermutohedralReverseGpu<T>::reverse(
...
template <typename T>
void PermutohedralReverseGpu<T>::max_reverse(
...
template <typename T>
__global__ void slice_tick_gpu_kernel(const int out_size, const T* sliced,
                                 const int M, const int d,
                                 const int out_offset,
                                 const int num_output,
                                 const int* offset, const T* barycentric,
...
template <typename T>
void PermutohedralReverseGpu<T>::blur_tick(
    const Blob<value_type>& blurred_tick,
    Blob<value_type>* blurred_out, value_type* filter_out) {
  ...
  for (std::size_t g = 0; g < group_; ++g) {
    for (std::size_t c = 0; c < chunks; ++c) {
      ...
      caffe_gpu_gemm_ex<value_type>(
          ...
          blurred_tick.gpu_data() + top_offset * g + chunk_size * c, N,
          static_cast<value_type>(0),
          col_diff_data, end - start);

      col2im(col_diff_data, value_size_ / group_, size,
             M_, start, end,
             blur_neighbors,
             blurred_out->mutable_gpu_data()
             + (value_size_ / group_) * g * (N + 1));
    }
  }
}

template <typename T>
__global__ void maxtick_gpu_kernel(
    const T* tick_data, const int value_size,
    const int filter_size, const int M,
    const int* blur_neighbors, T* max_out_tick_data,
    const int* idx_data) {
  CUDA_KERNEL_LOOP(i, value_size) {
    ...
    for (std::size_t j = 0; j < M; ++j) {
      ...
      for (std::size_t k = 1; k < filter_size; ++k) {
        ...
        if (idx_data[i * M + j] == i * (M+1) + neighbors[j] + 1){
          max_out_tick_data[i * (M+1) + neighbors[j] + 1] += tick_data[i * M + j];
        }
      }
    }
  }
...
template <typename T>
void PermutohedralReverseGpu<T>::max_tick(
    const Blob<value_type>& maxxed_tick,
  ...
  maxtick_gpu_kernel<value_type>
    // NOLINT_NEXT_LINE(whitespace/operators)
    <<<CAFFE_GET_BLOCKS(value_size_), CAFFE_CUDA_NUM_THREADS>>>(
        tick_data, value_size_, filter_size, M_,
        blur_neighbors, max_out_tick_data, max_idx_.gpu_data());

  CUDA_POST_KERNEL_CHECK;
}

template <typename T>
__global__ void splat_tick_gpu_kernel(const int in_size, const T* splatted,
                                      const int M, const int d,
                                      const int in_offset,
                                      const int value_size,
                                      const int* offset, const T* barycentric,
...
void PermutohedralReverseGpu<T>::splat_tick(
  ...
  splat_tick_gpu_kernel<value_type>
    ...
    (in_size_, splatted_tick.gpu_data(),
    M_, d_,
    in_offset_,
    value_size_,
    offset_.gpu_data(), barycentric_.gpu_data(),
...
template <typename T>
boost::shared_ptr<PermutohedralReverse<T> >
Permutohedral<T>::max_compute_gpu(const value_type* filter,
    const value_type* in, int value_size,
    int in_offset, int out_offset, int in_size, int out_size,
    value_type* out) {
...
template boost::shared_ptr<PermutohedralReverse<double> >
  Permutohedral<double>::compute_gpu(const double* filter, const double* in,
  int num_input, int group, int value_size, bool do_skip_blur,
  int in_offset, int out_offset, int in_size, int out_size,
      double* out) const;
template boost::shared_ptr<PermutohedralReverse<float> >
  Permutohedral<float>::compute_gpu(const float* filter, const float* in,
  int num_input, int group, int value_size, bool do_skip_blur,
  int in_offset, int out_offset, int in_size, int out_size,
      float* out) const;

  template boost::shared_ptr<PermutohedralReverse<double> >
  Permutohedral<double>::max_compute_gpu(const double* filter, const double* in,
  int value_size, int in_offset, int out_offset, int in_size, int out_size,
      double* out);
template boost::shared_ptr<PermutohedralReverse<float> >
  Permutohedral<float>::max_compute_gpu(const float* filter, const float* in,
  int value_size, int in_offset, int out_offset, int in_size, int out_size,
      float* out);
