...
void PermutohedralLayerTemplate<Dtype, Ptype>::Backward_gpu(
  ...
  Blob<Dtype>& bottom_feat_blob = *bottom[1];
  Dtype* const bottom_feat_diff = bottom_feat_blob.mutable_gpu_diff();
  
  caffe_gpu_memset(bottom_blob.count() * sizeof(Dtype), 0,
                   bottom_diff);
  caffe_gpu_memset(bottom_feat_blob.count() * sizeof(Dtype), 0,
                   bottom_feat_diff);
