...
__global__ void ReLU2Forward(const int n, const Dtype* in, Dtype* out,
...
template <typename Dtype>
void ReLU2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  ReLU2Forward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
template <typename Dtype>
__global__ void ReLU2Backward(const int n, const Dtype* in_diff,
...
template <typename Dtype>
void ReLU2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    Dtype threshold_ = this->layer_param_.relu2_param().threshold();
    // NOLINT_NEXT_LINE(whitespace/operators)
    ReLU2Backward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
INSTANTIATE_LAYER_GPU_FUNCS(ReLU2Layer);
